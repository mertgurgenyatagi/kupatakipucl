...
- Keeps the rolling-image hero carousel (currently on the Leaderboard page) once it's built for Home too — confirmed still belongs there (round 6). **Built** (chat-widget-round-01 Q1): reuses `leaderboard/HeroCarousel.tsx` as-is, sitting in the cell row between Forum and Sohbet.
...
**b. League prediction** (the alphabetical team-ranking drag-and-drop, `SPEC.md` §5 steps 3–4) — decoupled from the quiz. Submitted via its own dedicated `/predictions` page (round 9 — reverses round 4/6's "Home-triggered popup" call; see changelog), anytime between sign-up and kickoff — does *not* have to happen at sign-up itself. **Locks permanently once league phase starts** — no reopening, ever, after that. The existing overwrite-confirmation UX (§5 step 4) still applies to edits made before the lock. Home's "Tahminini Yap" button links straight there.
...
- `/predictions` route: **stays**, own dedicated page (round 9 reversal — see §5b/changelog). Not folded into Profile or a popup.
...
- **2026-07-25 (chat-widget-round-01):** A 24-question round on the Home chat widget (own file, `onboarding/pagemap-questionnaires/chat-widget-round-01.md`, same shape as forum-widget-round-01) — noted here only because two of its answers reverse or complete standing DECIDED items elsewhere. (1) **Reverses SPEC.md §6's chat permanence stance** — "permanent scrollback... no edits or deletes, ever, by anyone" — a sender can now soft-delete their own message (leaves a "Bu mesaj silindi." placeholder; still no editing, still nobody can touch anyone else's). (2) **Completes §3's long-standing hero-carousel note** — it's now actually built on logged-in Home, reusing `leaderboard/HeroCarousel.tsx`, sized at Mert's explicit request to 60% of an equal quarter-share with the other three cells splitting the rest evenly. Everything else this round decided (message cap + "load older" pagination, date dividers, per-message timestamps, WhatsApp-style consecutive grouping, a typing indicator, an online-count signal, @mentions, a 360-char cap, and an in-widget search toggle — with read receipts, an unread signal, sound/browser notifications, reactions, photos, GIFs, slow mode, moderation, pinned messages, and matchday-specific behavior all declined) is scoped entirely to the Home chat cell and doesn't touch any other DECIDED item in this document.
- **2026-07-25 (round 9):** League prediction reverses course — it's staying its own dedicated page (`/predictions`), not folding into a Home-triggered popup or Profile as round 4/6 had it. Raised mid-build of logged-in Home's "Tahminini Yap" button; the popup was never actually built, so nothing code-side needed undoing. `/chat`'s own planned deletion (§6) is unaffected — that's still on track, just not executed yet either.
