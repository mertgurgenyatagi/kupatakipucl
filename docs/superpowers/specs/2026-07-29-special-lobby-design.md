...
**Action functions**, one file each, matching the existing `src/forum/createPost.ts` convention: `createLobby()` (writes the lobby doc *and* the creator's own bootstrap `members/{uid}` doc — `viaInviteId: null` — as one batch, plus the `created` system message), `generateLobbyInvite(lobbyId)`, `joinLobbyViaInvite(inviteId)`, `leaveLobby(lobbyId)`, `removeMember(lobbyId, uid)`, `renameLobby(lobbyId, name)` (also writes the `renamed` system message), `deleteLobby(lobbyId)`.

**`leaveLobby()`'s three branches — this is where Round 1 Q10 and Round 5's solo-creator answer both live, and they cover different situations:**
1. The leaver is the creator *and* the only member left → the solo-creator-leaves case (Round 5): delete both their membership doc and the lobby doc itself. No system message — nobody remains to read it.
2. The leaver is the creator, but other members remain → Round 1 Q10's ownership transfer: `createdByUid` is updated to whichever remaining member has the earliest `joinedAt`, *then* the leaver's membership doc is deleted and a `left` system message is written. This is a real, separate code path from case 1 — not just a variant of it — and needed its own rule permission (below).
3. The leaver isn't the creator → delete their membership doc, write a `left` system message. No ownership change.
...
- Deleting a lobby *does* delete every `members/{uid}` doc as part of the same operation — this is required, not optional: every read rule in this feature (`lobbies`, `members`, `messages`) checks "does the requester's own member doc exist," not "does the lobby doc exist," so leaving member docs behind after deleting the lobby doc would let every former member keep reading its messages forever, defeating "wipes it for everyone" (Round 2 Q7). This is cheap and bounded (at most a few dozen small docs, known upfront via one `getDocs` on the members subcollection) — a genuinely different cost than the next point.
- What deletion does *not* cascade is the `messages` subcollection itself, or any `lobbyInvites` docs referencing the lobby. Once every member doc is gone, both become permanently unreadable anyway (the same read rules apply) but aren't actively cleaned up — avoids needing either a Cloud Function or client-side recursive batch-deletes for a potentially-unbounded message history, for data nobody can read again regardless.
...
- `lobbies/{lobbyId}`: `read` requires an existing `members/{request.auth.uid}` doc under this lobby; `create` open to any signed-in user with `name` validated (string, 1–15 chars) and `createdByUid == request.auth.uid`; `update` allows two distinct shapes — a rename (existing member doc required, `diff().affectedKeys().hasOnly(['name'])`, same name validation) **or** an ownership transfer (`diff().affectedKeys().hasOnly(['createdByUid'])`, `resource.data.createdByUid == request.auth.uid` — only the current creator can trigger this, as part of their own departure — and the new `createdByUid` must reference an existing member: `exists(/databases/$(database)/documents/lobbies/$(lobbyId)/members/$(request.resource.data.createdByUid))`); `delete` requires `resource.data.createdByUid == request.auth.uid`.
