...
- Filling in both existing reserved trigger points (`FixtureRow`, `TeamPopup`'s `MatchRow`) and threading the new `onSelectFixture` callback through every page that can reach either: `LeaderboardPage`, `ProfilePage`, `StatsPage` (via `StatsHero`), `HomeLandingLoggedIn` (via `HomeHero`), `HomeLandingLoggedOutStarted` (via `UpcomingMatchesPreview`).
- `LeaderboardHero` (the shared wrapper embedding `UpcomingMatchesDrawer`, reused by Leaderboard/Stats/logged-in-Home) gains an `onSelectFixture` prop, since it currently passes the drawer no click wiring at all.
...
**Parent-level wiring** (each gets a new `selectedFixtureId` state slot + a rendered `<MatchupPopup>`, mirroring the existing `selectedTeamId`/`selectedUid` pattern):

| Page | Reaches `MatchupPopup` via |
|---|---|
| `LeaderboardPage.tsx` | `LeaderboardHero` (drawer) + its own `TeamPopup` instance |
| `ProfilePage.tsx` | its own `TeamPopup` instance |
| `StatsPage.tsx` | `StatsHero` → `LeaderboardHero` (drawer) |
| `HomeLandingLoggedIn.tsx` | `HomeHero` → `LeaderboardHero` (drawer) |
| `HomeLandingLoggedOutStarted.tsx` | `UpcomingMatchesPreview` + its own `TeamPopup` instance |
...
- Page-level tests for all 5 wiring points in the table above: `MatchupPopup` renders and its state slot opens/closes correctly.
