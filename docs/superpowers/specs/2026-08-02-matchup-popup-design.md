...
- Filling in both existing reserved trigger points (`FixtureRow`, `TeamPopup`'s `MatchRow`) and threading the new `onSelectFixture` callback through every page that can actually reach either — see the corrected wiring table in §3. Verified against real code during planning, in two passes: `StatsPage` has no fixture list or popup infrastructure at all today; `HomeHero` (logged-in Home's hero cell) turns out to be `HeroCarousel` alone — same minimal shape as `StatsHero` — not a wrapper around `LeaderboardHero`/`UpcomingMatchesDrawer` as first assumed, and `HomeLandingLoggedIn` has no `TeamPopup` either. `UpcomingMatchesDrawer` is only ever reachable via `LeaderboardHero`, which is only ever rendered by `LeaderboardPage`. Net result: exactly 3 pages have any reserved trigger to fill in.
- `LeaderboardHero` (the wrapper embedding `UpcomingMatchesDrawer`, rendered only by `LeaderboardPage`) gains an `onSelectFixture` prop, since it currently passes the drawer no click wiring at all.
...
**Parent-level wiring** (each gets a new `selectedFixtureId` state slot + a rendered `<MatchupPopup>`, mirroring the existing `selectedTeamId`/`selectedUid` pattern). Verified against the real component tree during planning — see the note above. Exactly 3 pages have a reserved trigger to fill in:
...
| ~~`StatsPage.tsx`~~ | — | Excluded: no fixture list, no `TeamPopup`/`ParticipantPopup` on this page at all. |
| ~~`HomeLandingLoggedIn.tsx`~~ | — | Excluded: `HomeHero` is `HeroCarousel` alone (no drawer), and this page has no `TeamPopup` either — no reserved trigger exists here to fill in. |
...
- Page-level tests for all 3 wiring points in the table above: `MatchupPopup` renders and its state slot opens/closes correctly.
