 ...
 *  rather than "NaN dk önce", and a timestamp slightly in the future (client
 *  clock skew — createdAt is written with the poster's own Date.now(), not a
 *  server timestamp, see createPost.ts) clamps to "az önce" instead of
...
// rolls over — a post read as "az önce" stayed "az önce" for as long as the
